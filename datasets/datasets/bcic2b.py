    ...
    def __init__(
        ...
        if args.mode == "cls":
            if args.is_subject_independent:
                ...
                if is_test:
                    cur.execute(
                        f"SELECT * FROM MetaData WHERE MetaData.Sub == {target_subject} AND MetaData.Test == 1"
                    )
                else:
                    cur.execute(
                        f"SELECT * FROM MetaData WHERE MetaData.Sub == {target_subject} AND MetaData.Test == 0"
                    )
            ...
            raise ValueError("mode should be either 'cls' or 'ssl' or 'qe'")
